...
try{S.playbackRate=parseFloat(localStorage.getItem('dooxyz_playback_rate'))||1.0;}catch(e){S.playbackRate=1.0;}
try{var storedAutoNext = localStorage.getItem('dooxyz_auto_next');if(storedAutoNext!==null){S.autoNext = storedAutoNext==='true';}}catch(e){}
...
try {
    var storedVol = parseFloat(localStorage.getItem('dooxyz_volume'));
...
function applyVolume(vol) {
    ...
    S.volume = vol;
    if (AU) AU.volume = vol;
    try { localStorage.setItem('dooxyz_volume', String(vol)); } catch(e){}
    updateVolumeUI();
}
...
function toggleAutoNext(){
    ...
    try { localStorage.setItem('dooxyz_auto_next', S.autoNext); } catch(e) {}
...
function getLikedSongs(){
    try{return JSON.parse(localStorage.getItem('dooxyz_liked_songs')||'[]');}catch(e){return[];}
}
function saveLikedSongs(songs){
    try{localStorage.setItem('dooxyz_liked_songs',JSON.stringify(songs));}catch(e){}
}
...
function getLikedArtists(){
    try{return JSON.parse(localStorage.getItem('dooxyz_liked_artists')||'[]');}catch(e){return[];}
}
function saveLikedArtists(artists){
    try{localStorage.setItem('dooxyz_liked_artists',JSON.stringify(artists));}catch(e){}
}
...
function getUserPlaylists(){
    try{
        var pls=JSON.parse(localStorage.getItem('dooxyz_playlists')||'[]');
        var changed=false;
        ...
        if(changed){
            localStorage.setItem('dooxyz_playlists',JSON.stringify(pls));
        }
        return pls;
    }catch(e){return[];}
}
function saveUserPlaylists(pls){try{localStorage.setItem('dooxyz_playlists',JSON.stringify(pls));}catch(e){}}
...
function setPlaybackSpeed(speed) {
    ...
    try {
        localStorage.setItem('dooxyz_playback_rate', speed);
    } catch(e) {}
